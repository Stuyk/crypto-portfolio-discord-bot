...
registerCommand({ name: 'interval', command, description: '<interval> <m/h/d> - Change your periodic update interval to <interval> minutes hours days or weeks.' });
...
async function command(msg: Discord.Message) {
   ...
   switch (type){
       case "m":
           ...
           await msg.delete();
           await db.updatePartialData(data._id, {periodic: {...data.periodic, interval: interval * 60000}}, COLLECTIONS.CRYPTO);
           break;
       case "h":
           await msg.reply(`Switched interval to ${interval} hours`);
           await db.updatePartialData(data._id, {periodic: {...data.periodic, interval: interval * 60000 * 60}}, COLLECTIONS.CRYPTO);
           await msg.delete();
           break;
       case "d":
           await msg.reply(`Switched interval to ${interval} days`);
           await db.updatePartialData(data._id, {periodic: {...data.periodic, interval: interval * 60000 * 60 * 24}}, COLLECTIONS.CRYPTO);
           await msg.delete();
           break;
       case "w":
           await msg.reply(`Switched interval to ${interval} weeks`);
           await db.updatePartialData(data._id, {periodic: {...data.periodic, interval: interval * 60000 * 60 * 24 * 7}}, COLLECTIONS.CRYPTO);
           await msg.delete();
           break;
       default:
